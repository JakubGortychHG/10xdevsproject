...
describe("SourceTextInputForm", () => {
  ...
  it("renders correctly with empty initial text", () => {
    ...
    expect(
      screen.getByRole("button", { name: /generuj fiszki/i }),
    ).toBeDisabled();
  });
  ...
  it("disables the form when loading", () => {
    ...
    expect(
      screen.getByRole("button", { name: /generowanie\.\.\./i }),
    ).toBeDisabled();
  });
  ...
  it("shows error when text is too short", async () => {
    ...
    expect(
      screen.getByText(/tekst źródłowy musi zawierać co najmniej 1000 znaków/i),
    ).toBeInTheDocument();
  ...
  it("shows error when text is too long", async () => {
    ...
    // Check if error message is shown
    expect(
      screen.getByText(/tekst źródłowy nie może przekraczać 10000 znaków/i),
    ).toBeInTheDocument();
    expect(mockSubmit).not.toHaveBeenCalled();
  });
  ...
  it("resets error message when input changes", async () => {
    ...
    expect(
      screen.queryByText(
        /tekst źródłowy musi zawierać co najmniej 1000 znaków/i,
      ),
