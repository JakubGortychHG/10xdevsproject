...
describe("useGenerationView", () => {
  ...
  it("should handle editing proposals", async () => {
    ...
    expect(result.current.editingProposalId).toBe(
  ...
  it("should handle accepting proposal", async () => {
    ...
    expect(result.current.canSaveAccepted).toBe(false);
    expect(
      result.current.proposals.find((p) => p.id === firstProposalId)?.status,
    ).toBe("pending");
    expect(
      result.current.proposals.find((p) => p.id === secondProposalId)?.status,
    ).toBe("pending");
